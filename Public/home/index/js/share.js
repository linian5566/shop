...
function pub_reply() {
    ...
    easyloader.load('dialog', function () {
        dialog(title, 'url:' + reurl('share/goods.html?do=' + tp + '&gid=' + gid + '&asktype=' + asktype + '&inajax=1'), {height: 350});
        h_load();
    });
}
...
function cart_ctl(tp, options) {
    cart_exists = true;
    s_load();
    gid = typeof (gid) == 'undefined' ? 0 : gid;
    var opts = {
        action: 'updatecart',
        inajax: '1',
        pam: '',
...
$('.photoview').live({
    mouseover: function () {
        var div = $(this);
        var bigdiv = $('.big_photo');
        var src = div.attr('psrc');
        bigdiv.show();
        pos = divcenter(div, bigdiv, 1);
        bigdiv.find('img').remove();
        bigdiv.append("<img src='" + src + "-350-280-f.jpg' />");
        bigdiv.css({left: pos.left, top: pos.top});
    },
    mouseout: function () {
        $('.big_photo').hide();
    }
...
//right bar
function rtchange() {
    var rth = $(window).height() - 380;
    var toph = 130, rtup = 280;
    if (rth < rtup) {
        toph = 130 - (rtup - rth);
        if (toph < 30) {
            toph = 30;
...
if (app == 'goodslist') {
    ...
    fixbar('#gdmenu', 'fixed shadow', function () {
        $('#gdmenu a').each(function () {
            var href = $(this).attr('href');
            if (href.indexOf('#scoll') == -1) {
                $(this).attr('href', href + '#scoll');
            }
        });
    }, function () {
        $('#gdmenu a').each(function () {
            var href = $(this).attr('href');
            if (href.indexOf('#scoll') != -1) {
                $(this).attr('href', href.substr(0, href.length - 6));
            }
        });
    });
...
if (app == 'goodslist' || app == 'search') {
    ...
    $(".gimg-min li a").mouseover(function () {
        ...
        var hdiv = $(".hovtitle");
        if (hdiv.length == 0) {
            $("body").append("<div class=\"hovtitle\"></div>");
            hdiv = $(".hovtitle");
        }
        if ($(this).attr("title") != "") {
